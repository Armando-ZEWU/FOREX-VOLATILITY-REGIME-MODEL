...
*Document de travail — v7, backtest complet marqué comme fait (correction d'un oubli de suivi) ; nouveau chantier de triage et placement des figures pour préparation à la publication*
...
- [x] Fan chart séquentiel pour le test glissant — fait, sous forme de 4 images individuelles (une par jour, prix réel marqué en point), plus ajout de la comparaison ouverture/haut/bas/clôture révélant 2 dépassements sur 4 (pas 1) en tenant compte du plus bas intrajournalier. Voir `docs/rolling_test.md`.
- [x] Test de robustesse sur une fenêtre de stress historique (COVID janvier-avril 2020) — fait, via un MS-GARCH ré-estimé sur données strictement antérieures à 2020 (vrai test hors-échantillon), paramètres gelés sur 84 jours. Couverture empirique clôture-à-clôture : 91,7% (vs 95% nominal), test binomial unilatéral non significatif (p=0,127) — ni validation ni invalidation claire. Deux groupes de dépassements identifiés : régime pas encore réactif (avant le choc) vs régime déjà alerté à 98%+ mais dépassement quand même (hypothèse ν₂ mal identifié pré-2020, non vérifiée). En tenant compte du plus haut/bas intrajournalier (données OHLC indépendantes) : 19/84 jours (22,6%) touchent hors fourchette, contre 7/84 (8,3%) sur la seule clôture. Voir `docs/covid_robustness_test.md`.
- [x] Backtest complet (grand nombre de prévisions à 1 jour sur l'ensemble 2010-2026, taux de couverture empirique vs 95% nominal) — fait, via fenêtre extensive (expanding window, West 1996) : 14 ré-estimations annuelles complètes, ~3500 prévisions journalières à paramètres gelés. Résultat majeur : couverture empirique 95,50% (3270/3424), test binomial unilatéral p=0,919 — **première validation statistique réellement puissante du projet**, aucune preuve de sous-couverture. Reproductibilité confirmée (fold 8 = `ms_garch_pre2020.R` exactement). Deux limites documentées honnêtement : ν₂ quasi inestimable sur la quasi-totalité des folds (2 seulement sur 14 atteignent la significativité conventionnelle, après correction d'une fausse piste de corrélation initialement trouvée puis invalidée) ; anomalie du fold 6 (2018) entièrement résolue via la matrice de transition (régime 2 rare mais très collant, ~0,5 entrée attendue sur l'année testée). Voir `docs/full_backtest.md`.
...
## 9. Triage et placement des figures (préparation de la publication)

Avec les 11 documents détaillés désormais stables, un audit systématique de toutes les figures générées (~64 graphiques au départ, répartis sur les 11 documents) a été mené pour préparer une éventuelle version publiable (papier/portfolio) : `docs/triage_graphs.md` classe chaque graphique en 🟢 Essentiel (corps du texte), 🟡 Annexe/GitHub, ou 🔴 Supprimable (redondant ou mal conçu) ; `docs/Graphs_Ways.md` précise, pour chaque 🟢, l'emplacement exact (après quelle section) et pourquoi.

**Trois observations transversales issues de ce triage** :
1. Sur les quatre vérifications de couverture du projet (price_range n=1, rolling_test n=4, covid_robustness n=84, full_backtest n=3424), seule la dernière porte une vraie puissance statistique — c'est elle qui doit porter l'argument de calibration dans le corps du texte ; les trois autres restent des jalons méthodologiques (annexe).
2. `covid_robustness_test.md` garde sa place malgré `full_backtest.md`, mais pour un rôle différent : la meilleure illustration qualitative de *comment* le modèle se comporte en crise (groupes A/B, réactivité en retard), pas la meilleure preuve statistique.
3. `regime_hmm.md` et `ms_garch.md` restent deux méthodes concurrentes pour le même objectif, le projet ne tranchant pas formellement entre les deux (AIC préfère MS-GARCH, BIC préfère le HMM) — le triage suppose le HMM comme récit principal et MS-GARCH comme robustesse en annexe.

**Correction nécessaire lors de la vérification** : l'archive fournie contenait des dossiers dupliqués/en conflit pour 4 documents (deux générations différentes de graphiques pour `covid_robustness_test.md` sous des noms de dossiers différents ; numéros de fichiers dupliqués pour `ms_garch.md`, `price_range.md`, `rolling_test.md`). Chaque doublon a été examiné visuellement avant tranchage, pas supposé résolu par le nom de fichier seul :

- **ms_garch.md** : 4 graphiques absents du triage initial ajoutés au corps du texte — la visualisation des deux récursions de régime indépendantes (Haas et al., section 1), la comparaison visuelle MS-GARCH vs GARCH simple, et un graphique résolvant l'écart "39-100%" resté ouvert dans le triage initial (part de stress annuelle selon 3 définitions). Un graphique de comparaison AIC/BIC initialement classé 🟡 a été rétrogradé 🔴 après vérification : son échelle rendait l'écart qu'il prétendait montrer visuellement invisible ; remplacé par une version en écart (delta) qui montre effectivement les chiffres du texte (+13,51 / +18,18).
- **price_range.md** : un doublon du graphique de ratio stress/calme écrasait visuellement sa propre conclusion (barres partant de zéro sur un ratio variant de 1,88 à 2,09) — la version à axe zoomé conservée à la place.
- **rolling_test.md** : 2 graphiques absents du triage initial ajoutés — la trajectoire indexée base 100 de P(stress) vs largeur de fourchette (illustration la plus nette de la tension "réactif mais pas assez réactif"), et un schéma de flux dédié au calcul de distorsion fantôme (§5.2, canal B) rendant ce calcul immédiatement lisible.

**Décision en attente** : intégration effective des références d'images dans les documents sources, ou mise à jour de `triage_graphs.md`/`Graphs_Ways.md` pour refléter ces 9 ajustements — pas encore fait.
