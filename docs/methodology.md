...
- [ ] Construire une fourchette de prix opérationnelle (objectif (b) initial) à partir de la prévision GARCH/MS-GARCH — combine la relation de magnitude déjà établie (`magnitude_regression.md`) avec le régime courant (MS-GARCH retenu comme référence, voir discussion), pour produire un intervalle [P_min, P_max] réellement calculé, pas juste illustré par un exemple. Étape non encore réalisée malgré toutes les briques déjà validées.
