...
*Document de travail — v4, projet complet : GARCH, régimes (HMM et MS-GARCH), et fourchette de prix opérationnelle*
...
- [x] Construire une fourchette de prix opérationnelle (objectif (b) initial) à partir de la prévision GARCH/MS-GARCH — fait, via `Risk()` (package `MSGARCH`) sur la distribution prédictive complète (mélange des 2 régimes, pondéré par leurs probabilités actuelles), pas une simulation de tirages (`predict()` ne renvoyait pas les tirages malgré l'argument demandé). Résultat au 2026-09-11 : fourchette à 95% = [1,1533 ; 1,1675] (largeur 1,22%), cohérente avec un quantile Student's t (ν≈7) vérifié indépendamment. Distinction établie entre ce résultat rigoureux et une comparaison illustrative par régime (approximation assumée, non précise). Clarification importante : la prévision vise le prochain jour de trading de la série (pas littéralement "demain" calendaire), avec une limite documentée sur les chocs de week-end non observés par un modèle calibré sur des clôtures quotidiennes. Voir `docs/price_range.md`.
